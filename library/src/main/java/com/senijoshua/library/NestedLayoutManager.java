 ...
 * NestedLayoutManager MUST be used in conjunction with NestedAdapter.
 *
 * @see NestedAdapter
 */

@SuppressWarnings("unused")
public class NestedLayoutManager extends RecyclerView.LayoutManager {
    ...
    private static final String TAG = NestedLayoutManager.class.getSimpleName();
    ...
    public void onAdapterChanged(RecyclerView.Adapter oldAdapter, RecyclerView.Adapter newAdapter) {
        ...
        try {
            adapter = (NestedAdapter) newAdapter;
        } catch (ClassCastException e) {
            throw new ClassCastException("NestedLayoutManager must be used with a RecyclerView where the adapter is a kind of NestedAdapter");
        }
    ...
    public void onAttachedToWindow(RecyclerView view) {
        ...
        try {
            adapter = (NestedAdapter) view.getAdapter();
        } catch (ClassCastException e) {
            throw new ClassCastException("NestedLayoutManager must be used with a RecyclerView where the adapter is a kind of NestedAdapter");
        }
    }
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        for (int adapterPosition = firstViewAdapterPosition; adapterPosition < state.getItemCount(); adapterPosition++) {
            ...
            if (itemViewType == NestedAdapter.TYPE_HEADER) {
                headerViews.add(v);
            ...
            } else if (itemViewType == NestedAdapter.TYPE_GHOST_HEADER) {
    ...
    private View createSectionHeaderIfNeeded(RecyclerView.Recycler recycler, int sectionIndex) {
        ...
        if (adapter.doesSectionHaveHeader(sectionIndex)) {
            ...
            for (int i = 0, n = getChildCount(); i < n; i++) {
                ...
                if (getViewBaseType(view) == NestedAdapter.TYPE_HEADER && getViewSectionIndex(view) == sectionIndex) {
                    return view;
                }
            }
        ...
        if (dy < 0) {
            ...
            while (scrolled > dy) {
                ...
                if (firstViewAdapterPosition > 0 && scrolled > dy) {
                    ...
                    boolean isHeader = itemViewType == NestedAdapter.TYPE_HEADER;
                    ...
                    if (isHeader) {
                        ...
                        isHeader = itemViewType == NestedAdapter.TYPE_HEADER;
                    ...
                    int top;
                    boolean isGhostHeader = itemViewType == NestedAdapter.TYPE_GHOST_HEADER;
                    if (isGhostHeader) {
     ...
     * @return the viewholder for the first visible item (not header or childHeader)
     */
    @Nullable
    public NestedAdapter.ItemViewHolder getFirstVisibleItemViewHolder(boolean fullyVisibleOnly) {
        return (NestedAdapter.ItemViewHolder) getFirstVisibleViewHolderOfType(NestedAdapter.TYPE_ITEM, fullyVisibleOnly);
    }

    /**
     * @param fullyVisibleOnly if true, the search will be limited to the first header not hanging off top of screen
     * @return the viewholder for the first visible header (not item or childHeader)
     */
    @Nullable
    public NestedAdapter.HeaderViewHolder getFirstVisibleHeaderViewHolder(boolean fullyVisibleOnly) {
        return (NestedAdapter.HeaderViewHolder) getFirstVisibleViewHolderOfType(NestedAdapter.TYPE_HEADER, fullyVisibleOnly);
    }

    /**
     * @param fullyVisibleOnly if true, the search will be limited to the first childHeader not hanging off top of screen or partially obscured by a header
     * @return the viewholder for the first visible childHeader (not header or item)
     */
    @Nullable
    public NestedAdapter.ChildHeaderViewHolder getFirstVisibleChildHeaderViewHolder(boolean fullyVisibleOnly) {
        return (NestedAdapter.ChildHeaderViewHolder) getFirstVisibleViewHolderOfType(NestedAdapter.TYPE_CHILD_HEADER, fullyVisibleOnly);
    }

    @Nullable
    private NestedAdapter.ViewHolder getFirstVisibleViewHolderOfType(int baseType, boolean fullyVisibleOnly) {
        if (getChildCount() == 0) {
            return null;
        }
        ...
        if (baseType != NestedAdapter.TYPE_HEADER) {
            NestedAdapter.HeaderViewHolder firstHeader = getFirstVisibleHeaderViewHolder(false);
            if (firstHeader != null) {
    ...
    private void recycleViewsOutOfBounds(RecyclerView.Recycler recycler) {
        ...
        for (int i = 0; i < numChildren; i++) {
            ...
            if (getViewBaseType(view) != NestedAdapter.TYPE_HEADER) {
        ...
        for (int i = 0; i < numChildren; i++) {
            ...
            if (getViewBaseType(view) == NestedAdapter.TYPE_HEADER && !remainingSections.contains(sectionIndex)) {
    ...
    private View getTopmostChildView() {
        ...
        for (int i = 0, e = getChildCount(); i < e; i++) {
            ...
            // ignore headers
            if (getViewBaseType(v) == NestedAdapter.TYPE_HEADER) {
                continue;
            }
    ...
    private NestedAdapter.ViewHolder getViewViewHolder(View view) {
        return (NestedAdapter.ViewHolder) view.getTag(R.id.nested_section_view_holder_tag);
    }
    ...
    private class SmoothScroller extends LinearSmoothScroller {
        ...
        public PointF computeScrollVectorForPosition(int targetPosition) {
            return new PointF(0, NestedLayoutManager.this.computeScrollVectorForPosition(targetPosition));
        }
