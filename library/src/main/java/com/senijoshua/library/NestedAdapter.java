...
/**
 * NestedAdapter
 * Represents a list of sections, each containing a list of items and optionally a header and or childHeader item.
 * NestedAdapter may be used with a normal RecyclerView.LinearLayoutManager but is meant for use with
 * NestedLayoutManager to allow for sticky positioning of header items.
 * <p/>
 * When invalidating the adapter's contents NEVER use RecyclerView.Adapter.notify* methods. These methods
 * aren't aware of the section information and internal state of NestedAdapter. As such, please
 * use the NestedAdapter.notify* methods.
 * <p/>
 * NestedAdapter manages four types of items: TYPE_HEADER, TYPE_ITEM, TYPE_CHILD_HEADER and TYPE_GHOST_HEADER.
 * Headers are the optional first item in a section. A section then has some number of items in it,
 * and an optional childHeader. The ghost header is a special item used for layout mechanics. It can
 * be ignored by NestedAdapter subclasses - but it is made externally accessible just in case.
 */
...
public class NestedAdapter extends RecyclerView.Adapter<NestedAdapter.ViewHolder> {
    ...
    public static final int TYPE_CHILD_HEADER = 3;

    private static class Section {
        int adapterPosition;    // adapterPosition of first item (the header) of this sections
        int numberOfItems;      // number of items (not including header or childHeader)
        int length;             // total number of items in sections including header and childHeader
        boolean hasHeader;      // if true, sections has a header
        boolean hasChildHeader;      // if true, sections has a childHeader
    }
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public int getItemViewBaseType() {
            return NestedAdapter.unmaskBaseViewType(getItemViewType());
        }

        public int getItemViewUserType() {
            return NestedAdapter.unmaskUserViewType(getItemViewType());
        }
    ...
    /**
     * @param sectionIndex index of the section in question
     * @return true if this section has a childHeader
     */
    ...
    /**
     * For scenarios with multiple types of footers, override this to return an integer in range [0, 255] specifying a custom type for this childHeader.
     * The value you return here will be passes to onCreateChildHeaderViewHolder and onBindChildHeaderViewHolder as the 'userType'
     *
     * @param sectionIndex the childHeader's section
     * @return the custom type for this childHeader in range [0,255]
     */
    public int getSectionChildHeaderUserType(int sectionIndex) {
        return 0;
    }
    ...
    /**
     * Called when a ViewHolder is needed for a section childHeader view
     *
     * @param parent         The ViewGroup into which the new View will be added after it is bound to an adapter position.
     * @param childHeaderUserType If getSectionHeaderUserType is overridden to vend custom types, this will be the specified type
     * @return A new ChildHeaderViewHolder holding a childHeader view
     */
    ...
    /**
     * Called to display childHeader data for a particular section
     *
     * @param viewHolder     the view holder to update
     * @param sectionIndex   the index of the section containing the childHeader to update
     * @param footerUserType if getSectionChildHeaderUserType is overridden to provide custom childHeader types, this will be the type for this childHeader
     */
    public void onBindChildHeaderViewHolder(ChildHeaderViewHolder viewHolder, int sectionIndex, int footerUserType) {
    }
    ...
    /**
     * Return the adapter position corresponding to the childHeader of the provided section
     *
     * @param sectionIndex the index of the section
     * @return adapter position of that section's childHeader, or NO_POSITION if section does not have childHeader
     */
        ...
        if (notify) {
            ...
            for (int sectionIndex : selectionState.keySet()) {
                ...
                if (state.section) {
                    ...
                    if (state.childHeader) {
                        notifySectionChildHeaderChanged(sectionIndex);
                    }
                }
            }
        }
    ...
    /**
     * Set whether an entire section is selected. this affects ALL items (and childHeader) in section.
     *
    ...
    /**
     * Select the childHeader of a section
     *
     * @param sectionIndex index of section
     * @param selected     selection state
     */
    public void setSectionChildHeaderSelected(int sectionIndex, boolean selected) {
        SectionSelectionState state = getSectionSelectionState(sectionIndex);
    ...
    /**
     * Toggle selection of childHeader in a section
     *
     * @param sectionIndex index of section
     */
    public void toggleSectionChildHeaderSelection(int sectionIndex) {
        setSectionChildHeaderSelected(sectionIndex, !isSectionChildHeaderSelected(sectionIndex));
    }

    /**
     * Check whether childHeader of a section is selected, or if the entire section is selected
     *
     * @param sectionIndex section index
     * @return true if the childHeader is selected
     */
    public boolean isSectionChildHeaderSelected(int sectionIndex) {
        SectionSelectionState state = getSectionSelectionState(sectionIndex);
        return state.section || state.childHeader;
    }
    ...
    /**
     * Notify that a section has had a childHeader added to it
     *
     * @param sectionIndex position of the section
     */
    public void notifySectionChildHeaderInserted(int sectionIndex) {
        if (sections == null) {
            buildSectionIndex();
            notifyAllSectionsDataSetChanged();
        } else {
            ...
            if (!section.hasChildHeader) {
                throw new IllegalArgumentException("notifySectionChildHeaderInserted: adapter implementation reports that section " + sectionIndex + " does not have a childHeader");
            }
    ...
    /**
     * Notify that a section has had a childHeader removed from it
     *
     * @param sectionIndex position of the section
     */
    public void notifySectionChildHeaderRemoved(int sectionIndex) {
        if (sections == null) {
            buildSectionIndex();
            notifyAllSectionsDataSetChanged();
        } else {
            ...
            Section section = this.sections.get(sectionIndex);
            if (section.hasChildHeader) {
                throw new IllegalArgumentException("notifySectionChildHeaderRemoved: adapter implementation reports that section " + sectionIndex + " has a childHeader");
            }
    ...
    /**
     * Notify that a section's childHeader's content has changed
     *
     * @param sectionIndex position of the section
     */
    public void notifySectionChildHeaderChanged(int sectionIndex) {
        if (sections == null) {
            buildSectionIndex();
            notifyAllSectionsDataSetChanged();
        } else {
            ...
            Section section = this.sections.get(sectionIndex);
            if (!section.hasChildHeader) {
                throw new IllegalArgumentException("notifySectionChildHeaderChanged: adapter implementation reports that section " + sectionIndex + " does not have a childHeader");
            }
    ...
    private void buildSectionIndex() {
        ...
        for (int s = 0, ns = getNumberOfSections(); s < ns; s++) {
            ...
            section.hasChildHeader = doesSectionHaveChildHeader(s);
    ...
    public int getItemViewType(int adapterPosition) {
        ...
        switch (baseType) {
            ...
            case TYPE_CHILD_HEADER:
                userType = getSectionChildHeaderUserType(sectionIndex);
                if (userType < 0 || userType > 0xFF) {
                    throw new IllegalArgumentException("Custom childHeader view type (" + userType + ") must be in range [0,255]");
                }
                break;
        }
     ...
     * @return the base type (TYPE_HEADER, TYPE_GHOST_HEADER, TYPE_ITEM, TYPE_CHILD_HEADER) of the item at a given adapter position
     */
    ...
    int getItemViewBaseType(Section section, int localPosition) {
        if (section.hasHeader && section.hasChildHeader) {
            if (localPosition == 0) {
                return TYPE_HEADER;
            ...
            // this sections has no header or childHeader
            return TYPE_ITEM;
        }
    }
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        throw new IndexOutOfBoundsException("unrecognized viewType: " + viewType + " does not correspond to TYPE_ITEM, TYPE_HEADER or TYPE_CHILD_HEADER");
    }
    ...
    public void onBindViewHolder(ViewHolder holder, int adapterPosition) {
        ...
        switch (baseType) {
            ...
            default:
                throw new IllegalArgumentException("unrecognized viewType: " + baseType + " does not correspond to TYPE_ITEM, TYPE_HEADER, TYPE_GHOST_HEADER or TYPE_CHILD_HEADER");
        }
    }
